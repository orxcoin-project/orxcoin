Orxcoin Core version 0.18.1 is now available from:

  <https://download.orxcoin.org/orxcoin-0.18.1/>
...
`/Applications/Orxcoin-Qt` (on Mac) or `orxcoind`/`orxcoin-qt` (on
Linux).
...
Orxcoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Orxcoin Core on unsupported systems.
...
10.10. Additionally, Orxcoin Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/orxcoin-project/orxcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@orxcoin.org) the Orxcoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 ORX),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in ORX per kilobyte, not litoshi per byte.

Documentation
-------------

- A new short [document](https://github.com/orxcoin-project/orxcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/orxcoin-project/orxcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/orxcoin-project/orxcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/orxcoin-project/orxcoin/blob/master/doc/orxcoin-conf.md)
  about the `orxcoin.conf` file describes how to use it to configure
  Orxcoin Core.

- A new document introduces Orxcoin Core's BIP174 [Partially-Signed
  Orxcoin Transactions
  (PSBT)](https://github.com/orxcoin-project/orxcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/orxcoin-project/orxcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Orxcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Orxcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/orxcoin-project/orxcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/orxcoin-project/orxcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Orxcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Orxcoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/orxcoin-project/orxcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/orxcoin-project/orxcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart orxcoind with the `-deprecatedrpc=generate`
...
- A new `orxcoin-wallet` tool is now distributed alongside Orxcoin
...
This section describes planned changes to Orxcoin Core that may affect
other Orxcoin software and services.

- Since version 0.16.0, Orxcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Orxcoin Core 0.20 (expected about
  a year after 0.18), Orxcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Orxcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Orxcoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Orxcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Orxcoin Core
...
- A sub-project of Orxcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Orxcoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Orxcoin Core's own implementation, although entropy
  gathered by Orxcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Orxcoin Core a
...
- On macOS, Orxcoin Core now opts out of application CPU throttling
