...
The Orxcoin Core daemon can be configured to act as a trusted "border
router", implementing the orxcoin wire protocol and relay, making
...
The ZeroMQ feature in Orxcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building orxcoind:
...
For instance:
    ...
    $ orxcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/orxcoind.tx.raw \
...
These options can also be provided in orxcoin.conf.
...
From the perspective of orxcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into orxcoind directly. Furthermore, no information is
...
using. Orxcoind appends an up-counting sequence number to each
